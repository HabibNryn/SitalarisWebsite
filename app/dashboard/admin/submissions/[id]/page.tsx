...
export default async function SubmissionDetailPage({ params }: PageProps) {
  ...
  const dataPewaris =
    typeof submission.dataPewaris === "string"
      ? JSON.parse(submission.dataPewaris)
      : submission.dataPewaris;

  const ahliWaris =
    typeof submission.ahliWaris === "string"
  ...
  // Lookup UploadFile untuk KTP (pewaris + ahli waris)
  const ktpUploadIds = [
    typeof dataPewaris?.ktpFile === "string" ? dataPewaris.ktpFile : null,
    ...ahliWarisList
      .map((a) => (typeof a.ktpFile === "string" ? a.ktpFile : null))
      .filter(Boolean),
  ].filter((v): v is string => Boolean(v));

  const uploadFiles = ktpUploadIds.length
    ? await prisma.uploadFile.findMany({
        where: { id: { in: ktpUploadIds } },
        select: { id: true, filePath: true, filename: true },
      })
    : [];

  const uploadFileById = new Map(uploadFiles.map((u) => [u.id, u]));
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <Badge
        className={`${colors[status] || "bg-gray-100"} px-3 py-1 font-medium`}
      >
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 font-medium">
                    {dataPewaris.namaAyah || "-"}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">Tempat/Tanggal Lahir</p>
                  <p className="mt-1 font-medium">
                    {dataPewaris.tempatLahir || "-"},{" "}
                    {dataPewaris.tanggalLahir
                ...
                <div>
                  ...
                  <p className="mt-1 font-medium">
                    {dataPewaris.pekerjaan || "-"}
                  </p>
                </div>

                <div className="col-span-2">
                  <p className="text-sm text-gray-500">KTP Pewaris</p>
                  {typeof dataPewaris?.ktpFile === "string" &&
                  uploadFileById.get(dataPewaris.ktpFile) ? (
                    (() => {
                      const up = uploadFileById.get(dataPewaris.ktpFile)!;

                      const buildFileHref = (filePath?: string | null) => {
                        if (!filePath) return "";

                        const normalized = filePath.replace(/\\/g, "/").trim();

                        // filePath sudah URL path publik
                        if (normalized.startsWith("/")) return normalized;

                        // filePath kemungkinan sudah "uploads/xxx" (tanpa leading slash)
                        if (normalized.startsWith("uploads/")) {
                          return `/${normalized}`;
                        }

                        // filePath kemungkinan "public/uploads/xxx"
                        if (normalized.startsWith("public/")) {
                          const withoutPublic = normalized.replace(/^public\//, "");
                          // contohnya: uploads/xxx -> /uploads/xxx
                          return withoutPublic.startsWith("/")
                            ? withoutPublic
                            : `/${withoutPublic}`;
                        }
                      ...
                      const href = buildFileHref(up.filePath);
                      return (
                        <div className="mt-2 flex items-center gap-3">
                          ...
                          {href ? (
                            ...
                            <span className="text-sm text-gray-500">
                              File terdaftar, namun URL belum valid
                            </span>
                          )}
                        </div>
                      );
              ...
              <div className="space-y-4">
                {ahliWarisList.map((ahli, index) => (
                  <div key={index} className="border rounded-lg p-4">
                    ...
                    <div className="mt-3">
                      <p className="text-sm text-gray-500">KTP Ahli Waris</p>
                      {typeof ahli.ktpFile === "string" &&
                      uploadFileById.get(ahli.ktpFile) ? (
                        (() => {
                          const up = uploadFileById.get(ahli.ktpFile)!;
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              >
                <Link
                  href={`/api/admin/submissions/${id}/pdf/download`}
                  target="_blank"
                >
              ...
              >
                <Link
                  href={`/api/admin/submissions/${id}/pdf/preview`}
                  target="_blank"
                >
