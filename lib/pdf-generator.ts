...
export async function generateSuratPernyataanPDF(
  ...
  const addText = (text: string, size: number = 12, bold: boolean = false, x: number = margin) => {
    page.drawText(text, {
      x,
      y,
      size,
...
const checkNewPage = () => {
  ...
  addText(`Almarhum ${data.dataPewaris.nama} BIN ${data.dataPewaris.namaAyah|| '__________'}`, 12, true)
  y -= 10
  
  addText('Dengan ini menyatakan dengan sesungguhnya dan sanggup diangkat sumpah, bahwa:', 12)
  y -= 10
  ...
  addText(`1. Almarhum ${data.dataPewaris.nama} BIN ${data.dataPewaris.namaAyah || '__________'}`, 12)
  ...
  addText(`   Sesuai Akta Kematian No: ${data.dataPewaris.nomorAkteKematian || '__________'}`, 12)
  y -= 10
  ...
  if (istriList.length > 0) {
    istriList.forEach((istri, index) => {
      addText(`   Menikah dengan ${istri.nama} BINTI ${istri.namaAyah || '__________'}`, 12)
    })
  } else if (suamiList.length > 0) {
    suamiList.forEach((suami, index) => {
      addText(`   Menikah dengan ${suami.nama} BIN ${suami.namaAyah || '__________'}`, 12)
    })
  }
  y -= 10
  ...
  if (anakList.length > 0) {
    addText('3. Dari perkawinan tersebut dikaruniai anak yang masih hidup:', 12)
    anakList.forEach((anak, index) => {
      checkNewPage()
      addText(`   ${index + 1}. ${anak.nama}`, 12)
      addText(`      Tempat/Tgl Lahir: ${anak.tempatLahir}, ${formatDate(anak.tanggalLahir)}`, 12)
  ...
  // Penutup
  addText('Demikian surat pernyataan ini kami buat dengan sebenar-benarnya dan dalam', 12)
  addText('keadaan sehat jasmani dan rohani, serta tidak ada tekanan atau paksaan', 12)
  addText('dari siapapun/pihak manapun untuk dipergunakan sebagaimana mestinya.', 12)
  y -= 10
  
  addText('Apabila tidak benar atau dikemudian hari ada ahli waris lainnya selain', 12)
  addText('dari kami dan mempunyai bukti-bukti yang dianggap sah secara hukum atau', 12)
  addText('belum tercantum dalam surat pernyataan ini, maka kami selaku ahli waris', 12)
  addText('yang menandatangani surat pernyataan ini, bertanggung jawab sepenuhnya', 12)
